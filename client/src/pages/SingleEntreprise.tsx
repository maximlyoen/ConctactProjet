                      ...
                      {personnes.map((personne) => (
                          <div key={personne.ID_CONTACTS}>
                              <div className="flex">
                                  ...
                                  <p onClick={() => handleClick(personne.ID_CONTACTS)}>{personne.NOM} {personne.PRENOM}</p>
                              </div>
                              <div className="flex">
                                          ...
                                          <div key={tagInfo.contactId} className="flex">
                                              <div className="flex">
                                                  {tagInfo.tags.map((tag: any) => (
                                                    <div>
                                                      {tag.ID_TAG !== null && (
                                                      <div key={tag.ID_TAG} className="bg-gray-500 text-white font-bold py-2 m-2 h-auto rounded">
                                                          <p className="mr-2 ml-2">{tag.TAG_NOM_TAG}</p>
                                                          {tag.ID_TAG === 3 && tag.TAG_PRIX_TA !== null && (
                                                              <p className="mr-2 ml-2">{tag.TAG_PRIX_TA} €</p>
                                                          )}
                                                          {tag.ANNEE !== 0 ? <p className="mr-2 ml-2">{tag.ANNEE}</p> : <p className="mr-2 ml-2">Année inconnue</p>}
                                                      </div>
                                                      )}
                                                    </div>
                                                  ))}
